...
public class GameDatabase
{
    ...
    public static readonly ParameterData[] Prm_PlayerGrow = new ParameterData[] {
        ...
        new ParameterData(80, 60, 35, 55, 40, 20, 35, 20, 0),
        new ParameterData(60, 40, 40, 50, 65, 40, 25, 40, 0),
    ...
    {
        new(12, 8, 0, 4, 5, 9, 5, 5, 4),   // グリーンスライム
        new(15, 8, 0, 5, 6, 4, 5, 3, 4),   // スケルトン（剣）
        new(15, 9, 0, 5, 3, 3, 10, 0, 3),   // スケルトン（槍）
        new(15, 10, 0, 2, 5, 1, 6, 3, 4),   // スケルトン（斧）
        new(13, 7, 0, 6, 8, 4, 5, 3, 4),   // スケルトン（弓）
        new(13, 0, 9, 5, 5, 2, 3, 7, 4),   // スケルトン（魔）
        new(8, 0, 10, 4, 6, 9, 1, 10, 5),   // ペル
        new(10, 11, 0, 7, 5, 5, 7, 2, 4),   // バーンスライム
        new(9, 8, 0, 7, 10, 5, 5, 4, 6),   // エレキスライム
        new(15, 0, 10, 5, 5, 4, 5, 8, 2),   // ディープスライム

        new(24, 12, 0, 12, 2, 8, 20, 16, 1),   // メタルスライム

        new(20, 11, 0, 9, 7, 9, 7, 7, 4),   // エンジェルス
        new(20, 11, 0, 9, 7, 9, 7, 7, 4),   // アークエンジェルス
        new(20, 11, 0, 9, 7, 9, 8, 8, 4),   // プリンシパリティーズ
        new(20, 12, 0, 9, 7, 9, 8, 7, 5),   // パワーズ
        new(20, 12, 0, 10, 8, 9, 9, 7, 5),   // ヴァーチャーズ
        new(20, 12, 0, 10, 8, 10, 9, 8, 5),   // ドミニオンズ
        new(20, 12, 0, 10, 8, 10, 8, 9, 6),   // トロウンズ
        new(20, 0, 13, 10, 8, 10, 9, 10, 6),   // ケルビム
        new(20, 13, 0, 10, 8, 10, 10, 9, 6),   // セラフィム
        new(20, 14, 14, 11, 8, 11, 10, 11, 7),   // 『尊きもの』
    };

    /// <summary>敵成長率％</summary>
    public static readonly ParameterData[] Prm_EnemyGrow = new ParameterData[]
    {
        new(75, 50, 1, 25, 35, 55, 30, 30, 0),     // グリーンスライム
        new(90, 50, 1, 35, 40, 25, 35, 20, 0),     // スケルトン（剣）
        new(95, 55, 1, 30, 20, 20, 65, 5, 0),      // スケルトン（槍）
        new(95, 60, 1, 15, 35, 10, 40, 20, 0),     // スケルトン（斧）
        new(80, 45, 1, 40, 50, 25, 30, 20, 0),     // スケルトン（弓）
        new(80, 1, 55, 30, 35, 15, 20, 45, 0),     // スケルトン（魔）
        new(50, 1, 60, 25, 40, 55, 10, 65, 0),     // ペル
        new(65, 70, 1, 45, 35, 35, 45, 15, 0),     // バーンスライム
        new(55, 50, 1, 45, 60, 35, 35, 25, 0),     // エレキスライム
        new(95, 1, 65, 30, 30, 25, 30, 50, 0),     // ディープスライム
        new(120, 60, 1, 60, 10, 40, 100, 80, 0),   // メタルスライム

        new(100, 55, 1, 45, 35, 45, 35, 35, 0),    // エンジェルス
        new(100, 55, 1, 45, 35, 45, 35, 35, 0),    // アークエンジェルス
        new(100, 55, 1, 45, 35, 45, 40, 40, 0),    // プリンシパリティーズ
        new(100, 60, 1, 45, 35, 45, 40, 35, 0),    // パワーズ
        new(100, 60, 1, 50, 40, 45, 45, 35, 0),    // ヴァーチャーズ
        new(100, 60, 1, 50, 40, 50, 45, 40, 0),    // ドミニオンズ
        new(100, 60, 1, 50, 40, 50, 40, 45, 0),    // トロウンズ
        new(100, 1, 65, 50, 40, 50, 45, 50, 0),    // ケルビム
        new(100, 65, 1, 50, 40, 50, 50, 45, 0),    // セラフィム
        new(100, 70, 70, 50, 40, 55, 50, 55, 0),   // 『尊きもの』
    };
    ...
    public static readonly EnemyOtherData[] Prm_EnemyOther = new EnemyOtherData[]
    {
        ...
        new(ItemType.Sword, 4),         // スケルトン（剣）
        new(ItemType.Spear, 7),         // スケルトン（槍）
        new(ItemType.Axe, 4),           // スケルトン（斧）
        new(ItemType.Arrow, 11),         // スケルトン（弓）
        new(ItemType.Book, 13),         // スケルトン（魔）
        new(ItemType.None, 15),         // ペル
